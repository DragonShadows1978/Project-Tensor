DISPATCH CONTEXT (lead, 2026-09-06): You are the implementation seat for APA-SPD1. YOUR WRITABLE TARGET is this worktree: /mnt/ForgeRealm/Project-Tensor-wt-apa-spd1 (branch apa-spd1, forked from main). Never touch /mnt/ForgeRealm/Project-Tensor (main) or other worktrees. Your sandbox has NO GPU: build the bench, CPU-verify, deliver a leased bounded runner + blocked-report; the lead runs the card. Read AGENTS.md in this repo and /mnt/Shared/HOUSE_RULES.md first — the Prior Art Directive is binding. SP1 and E1 receipts are under artifacts/ here. NO git. NO subagents. NO background waits (foreground, explicit timeout, each call under 10 minutes; python3 -c 'import time;time.sleep(n)' for gaps). Never kill a process you did not start. Registration immutable; amendments separate. State your model id and effort. The order follows verbatim.
