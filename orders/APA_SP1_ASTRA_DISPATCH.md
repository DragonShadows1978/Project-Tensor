DISPATCH CONTEXT (lead, 2026-09-05): You are the implementation seat for APA-SP1. YOUR WRITABLE TARGET is this worktree: /mnt/ForgeRealm/Project-Tensor-wt-apa-sp1 (branch apa-sp1, forked from main @ 1c21280). Never touch /mnt/ForgeRealm/Project-Tensor (the main checkout) or any other worktree. Your sandbox has NO GPU: build and CPU-verify; deliver every GPU gate as a leased, bounded, runnable script plus a blocked-report for the lead. Reasoning effort: ultra is authorized for this order. NO git. NO subagents. NO background waits (foreground commands with explicit timeout, each under 10 minutes; use python3 -c 'import time;time.sleep(n)' if you need a gap, never bare sleep). Never kill a process you did not start. Registration file immutable; amendments separate. State your model id and effort in the final report. The order follows verbatim.
