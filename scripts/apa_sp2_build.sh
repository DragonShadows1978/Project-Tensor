...
set -euo pipefail
if [[ "${APA_SP2_BUILD_BOUNDED:-0}" != 1 ]]; then
  exec timeout --signal=TERM --kill-after=5s 540s env APA_SP2_BUILD_BOUNDED=1 bash "$0"
fi
ROOT=$(cd "$(dirname "$0")/.." && pwd -P)
[[ "$ROOT" == /mnt/ForgeRealm/Project-Tensor-wt-apa-sp2 ]] || exit 64
cd "$ROOT"
BUILD="$ROOT/artifacts/apa_sp2/build"
mkdir -p "$BUILD"
...
import hashlib,json
root=Path.cwd();build=root/'artifacts/apa_sp2/build'
...
PY
echo "Build complete in artifacts/apa_sp2/build"
