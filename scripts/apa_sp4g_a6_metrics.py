# A6 copy of the A4/A2 bitwise native replay and population metrics (2026).
...
def metric_tiles(c,rec,raw,native,sp,selected):
    ...
    for offset in range(0,rec['n'],128):
        ...
        f=save_array(A/'margin_errors_a6'/f'{c["id"]}_q{lo+offset:05d}.npy',err.ravel())
...
def whole_layer(c, deadline=float('inf')):
    os.environ['TC_APA_SP']='1';tc=load_runtime()
    import _apa_sp4g_diag as diag
    src=require_a6(c['capture_cell'])['result'];m=read(R/src['manifest'])
    ...
    for rec in recs:
        if time.monotonic() >= deadline:raise Red('A6_COOPERATIVE_MARGIN_RAIL')
