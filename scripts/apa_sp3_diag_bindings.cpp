...
// Project-Tensor Tensor ABI. SP3 adds checked, separate diagnostic wrappers.
...
std::pair<NDArray,NDArray> selective(const NDArray&,const NDArray&,const NDArray&,const NDArray&,float,float,bool);
}
static void check(Tensor& t) {
 const auto& a=t.data();
 if(a.ndim()!=4 || !a.device.is_cuda() ||
    (a.dtype!=DType::Float32 && a.dtype!=DType::Float16 && a.dtype!=DType::BFloat16))
   throw std::runtime_error("SP3 diagnostics need rank4 CUDA float tensors");
 for(auto d:a.shape) if(d<=0 || d>32768) throw std::runtime_error("SP3 diagnostic extent");
}
PYBIND11_MODULE(_apa_sp3_diag,m) {
 m.def("bulk_scores",[](Tensor& q,Tensor& kq,double scale){
   check(q);check(kq);
   auto qs=q.shape(),ks=kq.shape();
   if(qs[3]!=96 || ks[3]!=96 || qs[0]!=ks[0] || qs[1]!=ks[1] ||
      q.data().dtype!=kq.data().dtype || q.data().device.index!=kq.data().device.index ||
      !std::isfinite(scale) || scale<=0)
     throw std::runtime_error("SP3 bulk score contract");
   return Tensor::make(apa_sp3::bulk_scores(q.data(),kq.data(),scale),false);
 });
 m.def("blend",[](Tensor& b,Tensor& r,double z,int Lq,int row0){
   ...
   if(b.shape()!=r.shape() || b.data().dtype!=r.data().dtype ||
     ...
     throw std::runtime_error("SP3 blend contract");
   auto o=apa_sp3::blend(b.data(),r.data(),z,Lq,row0);
 ...
 m.def("selective",[](Tensor& q,Tensor& k,Tensor& kq,Tensor& v,double scale,double z,bool causal){
   ...
   if(qs[3]!=96 || vs[3]!=96 || ks!=kq.shape() || ks[3]!=96 ||
      qs[0]!=ks[0] || qs[1]!=ks[1] || vs[0]!=ks[0] || vs[1]!=ks[1] || vs[2]!=ks[2] ||
      qs[2]<=1 || (causal && ks[2]<qs[2]) || !std::isfinite(scale) || scale<=0 || !std::isfinite(z))
     throw std::runtime_error("SP3 selective requires D=VD=96 MHA prefill");
   for(auto t:{&k,&kq,&v}) if(t->data().dtype!=q.data().dtype || t->data().device.index!=q.data().device.index)
     throw std::runtime_error("SP3 diagnostic dtype/device mismatch");
   auto o=apa_sp3::selective(q.data(),k.data(),kq.data(),v.data(),scale,z,causal);
   return py::make_tuple(Tensor::make(o.first,false),Tensor::make(o.second,false));
 });
}
