...
def compare(candidate, reference):
    ...
    denom = max(float(np.linalg.norm(a.astype(np.float64).ravel())), 1e-30)
...
def dense(q, k, v, scale=1., sink='none', causal=True,
    ...
    for b in range(B):
        for h in range(H):
            ...
            if sink == 'zero':
                denom += np.exp(-maximum)  # extra zero-logit, zero-value sink
            probs = exps / denom
            if staged_bf16:
...
def classify(call_results):
    ...
    if all(r['vs_standard']['fp32_agreement'] and r['vs_own_dense']['fp32_agreement'] for r in nominal):
