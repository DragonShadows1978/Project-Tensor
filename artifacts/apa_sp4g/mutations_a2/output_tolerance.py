...
def require_exact_replay(out,mask,raw):
    if not np.allclose(out,raw['out']):raise Red('NATIVE_REPLAY_NOT_BITWISE')
...
def metric_tiles(c,rec,raw,native,sp,selected):
    ...
    for offset in range(0,rec['n'],128):
        ...
        f=save_array(A/'margin_errors_a2'/f'{c["id"]}_q{lo+offset:05d}.npy',err.ravel())
...
def whole_layer(c):
    os.environ['TC_APA_SP']='1';tc=load_runtime()
    import _apa_sp4g_diag as diag
    src=require_a2(c['depends'][0])['result'];m=read(R/src['manifest'])
    if m['ppl_cell']!=c['depends'][0] or m['arm']!=c['arm'] or m['population_rows']!=c['population_rows']:raise Red('A2_PPL_CAPTURE_SOURCE')
    ...
    return dict(result,files=[f for row in rows for f in row['files']],original_calls=len(recs),
        selection_source=c['depends'][0],selection_bitwise=True,population_rows=at,
