...
import time
import numpy as np
from apa_sp3_common import ART, BUILD, ROOT, Red, load_runtime, publish, registration, sha


def last512(logits, ids):
    # Prior art: standard teacher-forced NLL/perplexity (Shannon, 1948).
    # This scoring convention MUST match recovered source; no off-by-one patch
    # is inferred from the words "last 512" alone.
    ids = np.asarray(ids)
    ...
    return {'ppl': math.exp(float(nll.mean())), 'mean_nll': float(nll.mean()), 'targets': 512,
...
class Model:
    ...
    def capture(self, q, k, kq, mask, causal, path_kind):
        B, H, L, D = q.shape
        S = k.shape[2]
    ...
    def blend(self, q, k, kq, v, group, scale, z, causal, blk):
        ...
        self.capture(q, k, kq, tc.cat(masks, dim=2), causal, 'B_native_blend')
        return out
    ...
    def forward(self, ids, score=True):
        ...
        result = {'wall_ms': elapsed * 1000, 'timing': 'wall; sync before/after; one cold call',
