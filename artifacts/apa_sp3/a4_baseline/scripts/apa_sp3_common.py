...
def verify_sources(external=True):
    r = registration()
    if sha(ROOT/'orders/APA_SP3_MODEL_TEST.md') != r['order_sha256']:
        raise Red('immutable order changed')
    ...
    if external:
        ...
        imports=ART/'adapter_import_cpu.json'
        if imports.exists():
            for m in read(imports)['modules'].values():
                if sha(m['path'])!=m['sha256']:
                    raise Red(f'adapter dependency changed: {m["path"]}')
    return r


def verify_weight_stat():
    j=read(ART/'weight_identity.json')
    p=Path(j['path']);s=p.stat()
    now={k:getattr(s,k) for k in ('st_dev','st_ino','st_size','st_mtime_ns','st_ctime_ns')}
    if now!=j['stat'] or j['sha256']!=registration()['model']['weight_blob_claim']:
        raise Red('checkpoint identity changed since full hash verification')


def protocol():
    """No CLI bypass: only the lead-authorized, content-pinned amendment.

    Prior art: SHA-256 content pinning / immutable registrations are standard
    reproducibility practice. New: binding the lead's 2026 PROTOCOL-2 order.
    """
    p = ART / 'protocol_amendment.json'
    if not p.exists():
        raise Red('G0 BLOCKED_PROTOCOL: registered protocol amendment missing')
    registration()
    j = read(p)
    if j.get('registration_sha256') != REG_SHA or j.get('status') not in ('RECOVERED_ORIGINAL', PROTOCOL2_STATUS):
        raise Red('protocol amendment is not bound to this registration and original source')
    # Pin the complete authorized manifest, so changing status, hashes or a
    # threshold cannot manufacture authorization (including a legacy downgrade).
    if sha(p) != PROTOCOL2_SHA:
        raise Red('forged or stale protocol amendment SHA')
    if j.get('status') == PROTOCOL2_STATUS:
        if (j.get('amendment_order_path') != str(PROTOCOL2_ORDER)
                or j.get('amendment_order_sha256') != PROTOCOL2_ORDER_SHA
                or sha(PROTOCOL2_ORDER) != PROTOCOL2_ORDER_SHA):
            raise Red('stale amendment order pin')
        if (j.get('feeding') != FEEDING or j.get('window') != 1024
                or j.get('scored') != 512 or j.get('n_windows') != 6
                or j.get('window_starts') != list(range(0,6144,1024))
                or j.get('nll_dtype') != 'float64'
                or j.get('g0',{}).get('absolute_tolerance') != .001
                or j.get('model') != registration()['model']):
            raise Red('PROTOCOL-2 scoring/model/gate contract changed')
        for path, digest in j['input_sha256'].items():
            if sha(path) != digest:
                raise Red(f'PROTOCOL-2 input pin failed: {path}')
    ...
    if j.get('status') == PROTOCOL2_STATUS and (len(ids) <= 32800 or len(ids) != j['token_count']):
        raise Red('PROTOCOL-2 needs the pinned complete stream with >32800 tokens')
...
def fingerprint():
    ...
    files += [ART/'weight_identity.json',ART/'adapter_import_cpu.json']
    files += sorted(ART.glob('amendment_*.json'))
